...
class PaymentController extends Controller
{
    ...
    public function getBill(Request $request)
    {
        ...
        try {
            $response = $client->request('GET', '/api/v1/orders/payment/getbill', [
                'query' => [
            ...
            if ($response->getStatusCode() == 200) {
                ...
                return response()->json([
                    'status' => 'error',
                    'message' => 'Unexpected HTTP status: ' . $response->getStatusCode() . ' ' . $response->getReasonPhrase()
                ]);
            }
    ...
    public function payBill(Request $request)
    {
        ...
        // Validate the inputs
        if (empty($bill_number) || empty($amount) || empty($period) || empty($customer_code) || 
            empty($publisher) || empty($service_code) || empty($signature) || empty($api_key)) {
            return response()->json([
                'status' => 'error',
                'message' => 'Missing required parameters.'
            ], 400);
        ...
        try {
            $response = $client->request('POST', '/api/v1/orders/payment/paybill', [
                'query' => [
    ...
    public function getPubliser(Request $request)
    {
        $service_code = $request->input('service_code', '');
        if(!$service_code){
            return show_error('require_valid', $service_code);
        }
        $dataElectric = (object) [
            "EVN" => "Điện lực Hà Nội",
